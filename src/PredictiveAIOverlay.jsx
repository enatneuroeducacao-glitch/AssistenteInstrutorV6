...
const PREDICTIVE_AI_VERSION = "1.0.2";
...
function PredictiveCard({ data, onClose }) {
  ...
  const waiting = !data?.hasPrediction;
  ...
  return (
    <div style={{ position:"fixed", right:18, bottom:18, zIndex:99999, width:"min(390px, calc(100vw - 36px))", background:"#07101b", color:"#fff", border:"1px solid rgba(85,191,239,.45)", borderRadius:16, boxShadow:"0 18px 45px rgba(0,0,0,.35)", padding:18, fontFamily:"Arial, sans-serif", boxSizing:"border-box" }}>
      <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"flex-start"}}>
        ...
        <button onClick={onClose} aria-label="Fechar" style={{background:"transparent",border:0,color:"#fff",fontSize:18,cursor:"pointer"}}>×</button>
      </div>
      {waiting ? (
        <>
          <div style={{marginTop:14,padding:14,borderRadius:10,background:"rgba(85,191,239,.08)",lineHeight:1.5}}>
            <strong>IA conectada.</strong><br/>Aguardando uma avaliação preditiva registrada para este instrutor/aluno.
          </div>
          <div style={{fontSize:10,opacity:.5,marginTop:12}}>Nenhum dado foi inventado. A previsão aparecerá após os dados elegíveis serem processados.</div>
        </>
      ) : (
        <>
          ...
          <div style={{marginTop:12,padding:12,borderRadius:10,background:"rgba(85,191,239,.08)"}}><div style={{fontSize:11,opacity:.7}}>TENDÊNCIA</div><div style={{fontWeight:800,marginTop:3}}>{data.trend || "—"}</div></div>
...
function App() {
  ...
  async function load(userId) {
    ...
    if (!ids.length) { setData({ hasPrediction:false }); return; }
    ...
    if (prediction) setData({ hasPrediction:true, hsi: prediction.hsi_current, projectedHsi: prediction.hsi_projected, riskLevel: prediction.risk_level, riskScore: prediction.risk_score, trend: prediction.trend, factors: prediction.top_factors || [], recommendations: prediction.recommendations || [] });
    else setData({ hasPrediction:false });
  }
  ...
  return <PredictiveCard data={data || { hasPrediction:false }} onClose={() => setClosed(true)} />;
}
