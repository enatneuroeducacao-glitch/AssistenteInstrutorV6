...
function LessonRunning({ user, lesson, onCompleted, onBack, readOnly = false }) {
  ...
  return (
    <div>
      ...
      <div className="panel" style={{padding:"8px 12px",marginBottom:"10px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:"4px",alignItems:"center"}}>
          {LESSON_PHASES.map(phase=>{const pc=getLessonPhaseClass(phase.id,currentPhase,status);const done=pc==="completed";const active=pc==="current";return <div key={phase.id} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"5px",padding:"6px 4px",borderRadius:"7px",background:active?"#fff7df":done?"#edf9ef":"#f7f8fa",color:active?"#8a5a00":done?"#216b35":"#7a8794",border:"1px solid "+(active?"#f1c15b":done?"#a9d8b0":"#e3e7ec"),fontSize:"10px",fontWeight:800}}><span style={{width:"20px",height:"20px",borderRadius:"50%",display:"inline-flex",alignItems:"center",justifyContent:"center",background:active?"#f2b544":done?"#43a85b":"#dfe4ea",color:active||done?"#fff":"#6d7884",fontSize:"10px"}}>{done?"✓":phase.id}</span><span>{phase.label}</span></div>})}
        </div>
      </div>

      <div className="panel" style={{padding:"8px 12px",marginBottom:"10px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(90px,1fr))",gap:"4px"}}>{[["ALUNO",currentLesson?.ai_students?.full_name||currentLesson?.student_name||"—"],["VEÍCULO",currentLesson?.ai_vehicles?`${currentLesson.ai_vehicles.brand||""} ${currentLesson.ai_vehicles.model||""}`.trim():"—"],["STATUS",status==="completed"?"CONCLUÍDA":status==="paused"?"PAUSADA":"EM ANDAMENTO"],["KM",currentLesson?.km_start??"—"],["TEMPO",elapsedMinutes()!=null?`${elapsedMinutes()} min`:"—"]].map(([label,value])=><div key={label} style={{padding:"5px 8px",borderRight:"1px solid #e7ebf0"}}><div style={{fontSize:"9px",fontWeight:800,color:"#748398"}}>{label}</div><div style={{fontSize:"11px",fontWeight:900,marginTop:"2px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{value}</div></div>)}</div>
      </div>
      ...
      <details className="panel" open={currentPhase===4 && !evaluationComplete} style={{padding:"0",marginBottom:"8px"}}>
        <summary style={{cursor:"pointer",padding:"12px 14px",fontWeight:900}}>Avaliação andragógica <span style={{float:"right",fontSize:"10px",fontWeight:800}}>{evaluationComplete?`MÉDIA ${evaluationAverage.toFixed(2)} / 5`:"PENDENTE"}</span></summary>
        <div style={{padding:"0 12px 12px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:"6px"}}>{evaluationItems.map(item=><div key={item.key} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"6px",alignItems:"center",border:"1px solid #e2e7ed",borderRadius:"7px",padding:"7px"}}><div><div style={{fontSize:"11px",fontWeight:800}}>{item.label}</div><div style={{fontSize:"9px",color:"#718096"}}>{item.description}</div></div><div style={{display:"flex",gap:"2px"}}>{[1,2,3,4,5].map(value=>{const selected=Number(evaluation[item.key])===value;return <button key={value} type="button" onClick={()=>setEvaluationValue(item.key,value)} disabled={readOnly||busy||isFinished} style={{width:"26px",height:"26px",minWidth:"26px",padding:0,borderRadius:"5px",border:selected?"2px solid #2457a6":"1px solid #ccd4df",background:selected?"#e8f0ff":"#fff",fontWeight:selected?900:600}}>{value}</button>})}</div></div>)}</div>
        {evaluationComplete&&<div style={{padding:"8px 12px 12px",fontSize:"10px",color:"#52657a"}}><strong>Diagnóstico:</strong> {pedagogicalAnalysis.diagnosis}</div>}
      </details>

      <details className="panel" open={currentPhase===4 && !hsiComplete} style={{padding:"0",marginBottom:"8px"}}>
        <summary style={{cursor:"pointer",padding:"12px 14px",fontWeight:900}}>HSI-DOTH-P <span style={{float:"right",fontSize:"10px",fontWeight:800}}>{hsiComplete?`HSI ${(hsiAverage*20).toFixed(1)} / 100`:"PENDENTE"}</span></summary>
        <div style={{padding:"0 12px 12px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:"6px"}}>{hsiItems.map(item=><div key={item.key} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"6px",alignItems:"center",border:"1px solid #d8e4f4",borderRadius:"7px",padding:"7px"}}><div><div style={{fontSize:"11px",fontWeight:800}}>{item.label}</div><div style={{fontSize:"9px",color:"#718096"}}>{item.description}</div></div><div style={{display:"flex",gap:"2px"}}>{[1,2,3,4,5].map(value=>{const selected=Number(hsiEvaluation[item.key])===value;return <button key={value} type="button" onClick={()=>setHsiValue(item.key,value)} disabled={readOnly||busy||isFinished} style={{width:"26px",height:"26px",minWidth:"26px",padding:0,borderRadius:"5px",border:selected?"2px solid #2457a6":"1px solid #ccd4df",background:selected?"#e8f0ff":"#fff",fontWeight:selected?900:600}}>{value}</button>})}</div></div>)}</div>
      </details>

      <details className="panel" style={{padding:"0",marginBottom:"8px"}}>
        <summary style={{cursor:"pointer",padding:"12px 14px",fontWeight:900}}>Observações da aula <span style={{float:"right",fontSize:"10px",fontWeight:800}}>OPCIONAL</span></summary>
        <div style={{padding:"0 12px 12px"}}><textarea rows="2" value={notes} onChange={e=>setNotes(e.target.value)} disabled={readOnly||busy||isFinished} placeholder="Registre observações relevantes da aula..." /></div>
      </details>

      {message&&<div className="panel" style={{padding:"9px 12px",marginBottom:"8px"}}><p className="msg" style={{margin:0,fontSize:"11px"}}>{message}</p></div>}
    </div>
  );
}
