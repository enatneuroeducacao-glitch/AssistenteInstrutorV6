...
function LessonRunning({ user, lesson, onCompleted, onBack, readOnly = false }) {
  ...
  useEffect(() => {
    ...
    return (
    <div>
      <div className="panel" style={{position:"sticky",top:0,zIndex:20,padding:"14px 18px",marginBottom:"12px",background:"rgba(255,255,255,.98)",border:"1px solid #dfe7f2",boxShadow:"0 5px 16px rgba(20,50,80,.08)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"12px",flexWrap:"wrap"}}>
          <div><div style={{fontSize:"10px",fontWeight:900,letterSpacing:".08em",color:"#52708f"}}>AULAS / {readOnly ? "VISUALIZAÇÃO" : "AULA EM ANDAMENTO"}</div><h1 style={{margin:"3px 0 0",fontSize:"24px"}}>{readOnly ? "Visualização da aula" : "Aula em andamento"}</h1></div>
          {!readOnly && !isFinished ? <div style={{display:"flex",gap:"7px",alignItems:"center",flexWrap:"wrap"}}><RefreshButton />{status === "paused" ? <button type="button" onClick={resumeLesson} disabled={busy}>▶ RETOMAR</button> : <button type="button" onClick={pauseLesson} disabled={busy}>⏸ PAUSAR</button>}<button type="button" onClick={advancePhase} disabled={busy || status === "paused" || currentPhase >= 5} style={{fontWeight:900}}>{currentPhase >= 5 ? "FASE 5 — FINALIZAR" : "✓ AVANÇAR →"}</button></div> : <button type="button" onClick={onBack}>VOLTAR</button>}
        </div>
      </div>
      <div className="panel" style={{padding:"10px 14px",marginBottom:"12px"}}><div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(100px,1fr))",gap:"5px"}}>
        {[["ALUNO",currentLesson?.ai_students?.full_name || currentLesson?.student_name || "—"],["VEÍCULO",currentLesson?.ai_vehicles ? (currentLesson.ai_vehicles.brand || "")+" "+(currentLesson.ai_vehicles.model || "") : "—"],["STATUS",status === "completed" ? "CONCLUÍDA" : status === "paused" ? "PAUSADA" : "EM ANDAMENTO"],["KM INICIAL",currentLesson?.km_start ?? "—"],["DURAÇÃO",elapsedMinutes() != null ? elapsedMinutes()+" min" : "—"]].map(([label,value]) => <div key={label} style={{padding:"6px 9px",borderRight:"1px solid #e5ebf2"}}><div style={{fontSize:"9px",fontWeight:800,color:"#718096"}}>{label}</div><div style={{marginTop:"3px",fontWeight:800,fontSize:"12px"}}>{value}</div></div>)}
      </div></div>
      <div className="panel" style={{padding:"16px",marginBottom:"12px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"10px",marginBottom:"10px"}}><div><h2 style={{margin:"0 0 3px",fontSize:"18px"}}>Progresso da aula</h2><p style={{margin:0,fontSize:"11px",color:"#6b7785"}}>Fase {currentPhase} de 5 — {lessonPhaseLabel(currentPhase)}</p></div><span style={{fontSize:"10px",fontWeight:900,padding:"6px 9px",borderRadius:"999px",background:"#eef6ff",color:"#1b5fa7"}}>{currentPhase === 5 ? "PARADA SEGURA" : "EM ANDAMENTO"}</span></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(90px,1fr))",gap:"6px"}}>{LESSON_PHASES.map(phase => { const pc=getLessonPhaseClass(phase.id,currentPhase,status); const active=pc==="current"; const done=pc==="completed"; return <div key={phase.id} style={{padding:"9px 8px",borderRadius:"8px",border:"1px solid "+(active?"#f2b94b":done?"#8bc99a":"#e3a0a8"),background:active?"#fff9e8":done?"#f1faf3":"#fff3f4",color:active?"#8a5a00":done?"#216b35":"#b4232d"}}><div style={{fontSize:"9px",fontWeight:900}}>FASE {phase.id}</div><div style={{fontSize:"11px",fontWeight:900,marginTop:"3px"}}>{phase.label}</div><div style={{fontSize:"9px",marginTop:"3px"}}>{done?"✓ CONCLUÍDA":active?"● ATUAL":"○ PRÓXIMA"}</div></div>; })}</div>
      </div>
      <div className="panel" style={{padding:"16px",marginBottom:"12px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"10px",flexWrap:"wrap"}}><div><h2 style={{margin:"0 0 3px",fontSize:"18px"}}>Avaliação andragógica</h2><p style={{margin:0,fontSize:"11px",color:"#6b7785"}}>Avalie os cinco fatores de 1 a 5.</p></div><span style={{fontSize:"10px",fontWeight:900,padding:"6px 9px",borderRadius:"8px",background:evaluationComplete?"#eaf7ed":"#fff8e1",color:evaluationComplete?"#216b35":"#8a5a00"}}>{evaluationComplete ? "MÉDIA "+evaluationAverage.toFixed(2)+" / 5" : "PREENCHIMENTO PENDENTE"}</span></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:"7px",marginTop:"10px"}}>{evaluationItems.map(item => <div key={item.key} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"8px",alignItems:"center",border:"1px solid #e0e6ee",borderRadius:"8px",padding:"8px"}}><div><div style={{fontWeight:800,fontSize:"12px"}}>{item.label}</div><div style={{fontSize:"9px",color:"#718096",marginTop:"2px"}}>{item.description}</div></div><div style={{display:"flex",gap:"3px"}}>{[1,2,3,4,5].map(value => {const selected=Number(evaluation[item.key])===value;return <button key={value} type="button" onClick={()=>setEvaluationValue(item.key,value)} disabled={readOnly||busy||isFinished} style={{width:"29px",minWidth:"29px",height:"29px",padding:0,borderRadius:"6px",border:selected?"2px solid #2457a6":"1px solid #ccd4df",background:selected?"#e8f0ff":"#fff",fontWeight:selected?800:600}}>{value}</button>;})}</div></div>)}</div>
        {evaluationComplete && <div style={{marginTop:"9px",padding:"9px",background:"#f7faff",border:"1px solid #dfe7f2",borderRadius:"8px",fontSize:"11px"}}><strong>Diagnóstico:</strong> {pedagogicalAnalysis.diagnosis}</div>}
      </div>
      <div className="panel" style={{padding:"16px",marginBottom:"12px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"10px",flexWrap:"wrap"}}><div><h2 style={{margin:"0 0 3px",fontSize:"18px"}}>HSI-DOTH-P</h2><p style={{margin:0,fontSize:"11px",color:"#6b7785"}}>Registre os cinco fatores em escala de 1 a 5.</p></div><span style={{fontSize:"10px",fontWeight:900}}>{hsiComplete ? "HSI "+(hsiAverage*20).toFixed(1)+" / 100" : "PREENCHIMENTO PENDENTE"}</span></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:"7px",marginTop:"10px"}}>{hsiItems.map(item => <div key={item.key} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"8px",alignItems:"center",border:"1px solid #d8e4f4",borderRadius:"8px",padding:"8px"}}><div><div style={{fontWeight:800,fontSize:"12px"}}>{item.label}</div><div style={{fontSize:"9px",color:"#718096",marginTop:"2px"}}>{item.description}</div></div><div style={{display:"flex",gap:"3px"}}>{[1,2,3,4,5].map(value => {const selected=Number(hsiEvaluation[item.key])===value;return <button key={value} type="button" onClick={()=>setHsiValue(item.key,value)} disabled={readOnly||busy||isFinished} style={{width:"29px",minWidth:"29px",height:"29px",padding:0,borderRadius:"6px",border:selected?"2px solid #2457a6":"1px solid #ccd4df",background:selected?"#e8f0ff":"#fff",fontWeight:selected?800:600}}>{value}</button>;})}</div></div>)}</div>
      </div>
      <div className="panel" style={{padding:"16px",marginBottom:"12px"}}><h2 style={{margin:"0 0 3px",fontSize:"18px"}}>Parada segura</h2><p style={{margin:"0 0 10px",fontSize:"11px",color:"#6b7785"}}>Na fase 5, informe o KM final e conclua a aula.</p><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}><div><label>KM final</label><input type="number" min={currentLesson?.km_start ?? 0} step="0.1" value={kmFinal} onChange={e=>setKmFinal(e.target.value)} disabled={readOnly||busy||isFinished}/></div><div><label>Distância</label><div style={{padding:"10px 12px",border:"1px solid #ddd",borderRadius:"6px",background:"#fbfcfe"}}>{Number.isFinite(distance)?distance.toFixed(1)+" km":"—"}</div></div></div><div style={{marginTop:"9px"}}><label>Observações</label><textarea rows="3" value={notes} onChange={e=>setNotes(e.target.value)} disabled={readOnly||busy||isFinished}/></div>{!readOnly&&<div style={{display:"flex",justifyContent:"flex-end",marginTop:"10px"}}><button type="button" onClick={completeLesson} disabled={busy||isFinished||status==="paused"||currentPhase!==5}>CONCLUIR AULA</button></div>}</div>
...
`;
  ...
  useEffect(() => {
    ...
    async function loadSubscription() {
      if (!supabase || !user?.id) {
        if (active) { setSubscription(null); setSubscriptionLoading(false); }
        return;
      }
      setSubscriptionLoading(true);
      const { data, error } = await supabase
        .from("ai_subscriptions")
        .select("id, provider, plan_code, provider_subscription_id, checkout_url, status, amount_cents, currency, frequency, frequency_type, current_period_start, current_period_end, next_due_date, last_payment_status, last_payment_at, canceled_at, paused_at, created_at, updated_at")
        .eq("user_id", user.id)
      ...
      if (!active) return;
      if (error) {
        console.warn("Assinatura ainda não configurada no Supabase:", error.message);
    ...
    loadSubscription();
    const timer = setInterval(loadSubscription, 30000);
...
if (tab === "alunos" && !showStudentForm) {
  ...
  function content() {
    if (!subscriptionActive && protectedTabs.has(tab)) {
      return <AccessBlockedPage onSubscribe={() => setTab("assinatura")} status={subscription?.status || "none"} />;
    }
...
if (tab === "rpa") {
  return (
    ...
    if (tab === "assinatura") {
      return <SubscriptionPage user={user} subscription={subscription} onRefresh={() => { setSubscriptionLoading(true); setTimeout(() => window.location.reload(), 250); }} onBack={() => setTab("dashboard")} />;
    }
...
if (tab === "aulas" && !showLessonForm && !showLessonHistory && !activeLesson) {
  const hoje = new Date();
  const inicioHoje = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
  const fimHoje = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate() + 1);
  ...
  const runningOrPaused = agendaLessons.filter(lesson =>
    ["running", "paused"].includes(String(lesson.status || "").toLowerCase())
  );

  function nomeAlunoAula(lesson) {
    const aluno = Array.isArray(lesson.ai_students) ? lesson.ai_students[0] : lesson.ai_students;
    return aluno?.full_name || "Aluno não identificado";
  }
...
function CoursesPage({ user, subscriptionActive, onNeedSubscription, onBack }) {
  ...
  function generateCertificate() {
    ...
    const moduleScores = selectedCourse.modules.map((module, index) => ({
      number: index + 1,
      title: module.title,
      subtitle: module.subtitle || "",
      score: Number(passedAssessments[index]?.score || 0)
    }));
    const finalAverage = moduleScores.length
      ? Number((moduleScores.reduce((sum, item) => sum + item.score, 0) / moduleScores.length).toFixed(2))
      : 0;
    ...
    const rows = moduleScores.map(item => `
      <tr>
        <td class="num">${item.number}</td>
        <td><strong>${safe(item.title)}</strong><span class="module-subtitle">${safe(item.subtitle)}</span></td>
...
html, body { margin: 0; padding: 0; width: 100%; background: #eaf0f5; color: #172a46; }
body { font-family: Georgia, 'Times New Roman', serif; }
.sheet { width: 297mm; height: 210mm; padding: 10mm; margin: 0 auto; background: white; page-break-after: always; break-after: page; }
.sheet:last-of-type { page-break-after: auto; break-after: auto; }
.page { width: 100%; height: 190mm; border: 3px solid #2457a6; padding: 7mm; position: relative; background: #fff; overflow: hidden; }
.page::before { content: ""; position: absolute; inset: 4mm; border: 1px solid #b9dff1; pointer-events: none; }
.inner { position: relative; z-index: 1; width: 100%; height: 100%; padding: 8mm 11mm; }
.front { display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; }
.brand { font-family: Arial, sans-serif; font-size: 30px; font-weight: 900; letter-spacing: 7px; color: #2457a6; }
.subtitle { font-family: Arial, sans-serif; font-size: 11px; letter-spacing: 1.5px; color: #55BFEF; margin-top: 3px; text-transform: uppercase; }
h1 { font-size: 27px; margin: 18px 0 8px; letter-spacing: 2px; text-transform: uppercase; }
.present { font-family: Arial, sans-serif; font-size: 13px; color: #667780; }
.name { font-size: 31px; font-weight: bold; color: #172a46; margin: 8px 0 12px; border-bottom: 1px solid #9fb7c8; padding: 0 28px 7px; }
.text { max-width: 850px; font-size: 16px; line-height: 1.45; }
.course { font-size: 21px; font-weight: bold; color: #2457a6; margin: 7px 0; }
.meta { font-family: Arial, sans-serif; font-size: 10px; color: #71808d; margin-top: 14px; }
.signature-wrap { position: absolute; left: 15mm; bottom: 8mm; text-align: center; font-family: Arial, sans-serif; z-index: 5; display: block; }
.signature { font-family: "Segoe Script", "Brush Script MT", cursive; font-size: 23px; color: #183f88; line-height: 1; margin-bottom: 4px; }
.signature-line { width: 67mm; border-top: 1px solid #8c9aa5; padding-top: 4px; font-size: 9px; }
.seal { position: absolute; right: 17mm; bottom: 8mm; width: 30mm; height: 30mm; border: 3px solid #c79a2b; border-radius: 50%; display: grid; place-items: center; text-align: center; color: #2457a6; font-family: Arial, sans-serif; font-size: 8px; font-weight: 800; line-height: 1.15; }
.footer { position: absolute; right: 15mm; bottom: 8mm; font-family: Arial, sans-serif; font-size: 8px; color: #667780; text-align: right; }
.back .inner { padding: 7mm 9mm; }
.back-title { text-align: center; font-family: Arial, sans-serif; font-size: 16px; font-weight: 900; color: #2457a6; letter-spacing: 1px; margin-bottom: 5mm; }
.back-grid { display: grid; grid-template-columns: 58% 42%; gap: 7mm; }
.section-title { font-family: Arial, sans-serif; font-size: 11px; font-weight: 900; color: #2457a6; letter-spacing: .5px; margin: 0 0 7px; }
table { width: 100%; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 9px; }
th { background: #edf6fc; color: #173c70; text-align: left; padding: 6px; border: 1px solid #c9ddec; }
td { padding: 6px; border: 1px solid #d7e2ea; vertical-align: top; }
td.num { width: 7%; text-align: center; font-weight: 800; }
td.score { width: 12%; text-align: center; font-weight: 900; color: #2457a6; font-size: 11px; }
.module-subtitle { display: block; color: #71808d; margin-top: 2px; font-size: 8px; }
.average { margin-top: 8px; border: 1px solid #b9dff1; background: #f6fbfe; padding: 8px; display: flex; justify-content: space-between; align-items: center; font-family: Arial, sans-serif; font-weight: 800; }
.average strong { color: #2457a6; font-size: 15px; }
.legal { font-family: Arial, sans-serif; font-size: 8.5px; line-height: 1.45; color: #39495a; }
.legal-box { border: 1px solid #c9ddec; border-radius: 6px; padding: 8px; margin-bottom: 7px; background: #fbfdff; }
.legal-box strong { color: #173c70; }
.disclaimer { font-size: 8px; color: #667780; }
.info-row { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; margin-top: 7px; }
.info { border: 1px solid #d7e2ea; padding: 7px; font-family: Arial, sans-serif; font-size: 8px; }
.info strong { display: block; color: #2457a6; margin-bottom: 2px; }
.back-signature { position: absolute; left: 14mm; bottom: 7mm; text-align: center; font-family: Arial, sans-serif; }
.back-signature .signature { font-size: 20px; }
.back-signature .signature-line { width: 65mm; font-size: 8px; }
.registration { position: absolute; right: 14mm; bottom: 7mm; font-family: Arial, sans-serif; font-size: 7.5px; color: #667780; text-align: right; }
.actions { position: fixed; top: 12px; right: 12px; z-index: 100; font-family: Arial, sans-serif; }
...
.page-break + .sheet { page-break-before: always; break-before: page; }
@media print { body { background: white; } .sheet { margin: 0; width: 297mm; height: 210mm; page-break-after: always !important; break-after: page !important; } .sheet:last-of-type { page-break-after: auto !important; break-after: auto !important; } .page-break + .sheet { page-break-before: always !important; break-before: page !important; } .actions { display: none; } }
</style>
</head>
<body>
...
<div class="sheet page-break">
  <div class="page">
    <div class="inner front">
      ...
      <div class="text">com aprovação em todas as avaliações finais dos módulos, obtendo aproveitamento mínimo de <strong>7,0</strong> em cada avaliação.</div>
      <div class="meta">Emitido em ${safe(date)} • Certificado nº ${safe(certificateId)}</div>
      <div class="signature-wrap">
        <div class="signature">Vilmar Becker</div>
        <div class="signature-line"><strong>VILMAR BECKER</strong><br>Diretor e Responsável Técnico do ENAT</div>
      </div>
      <div class="seal">ENAT<br>CERTIFICADO<br>DE CONCLUSÃO</div>
      <div class="footer">Registro institucional do Assistente do Instrutor</div>
    </div>
  </div>
</div>
...
<div class="sheet">
  <div class="page back">
    <div class="inner">
      <div class="back-title">VERSO DO CERTIFICADO — CONTEÚDO PROGRAMÁTICO E DESEMPENHO</div>
      <div class="back-grid">
        <div>
          <div class="section-title">MÓDULOS / MATÉRIAS ESTUDADAS E NOTAS</div>
          <table>
            <thead><tr><th>#</th><th>Módulo / matéria estudada</th><th>Nota</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
          <div class="average"><span>MÉDIA FINAL DA FORMAÇÃO</span><strong>${finalAverage.toFixed(2)}</strong></div>
          <div class="info-row">
            <div class="info"><strong>APROVEITAMENTO</strong>Mínimo exigido: 7,0 por avaliação</div>
            <div class="info"><strong>MÓDULOS CONCLUÍDOS</strong>${moduleScores.length} de ${selectedCourse.modules.length}</div>
          </div>
        </div>
        <div class="legal">
          <div class="section-title">NATUREZA DA FORMAÇÃO E BASE LEGAL</div>
          <div class="legal-box"><strong>Natureza:</strong> formação de aprofundamento, atualização e aperfeiçoamento profissional, com certificação institucional de conclusão.</div>
          <div class="legal-box"><strong>Referências federais:</strong><br>• Lei nº 9.394/1996 — LDB, especialmente arts. 39, 40 e 42, que tratam da educação profissional e tecnológica, formação inicial e continuada/qualificação profissional e cursos especiais.<br>• Decreto nº 5.154/2004, especialmente arts. 1º e 3º, que regulamenta a educação profissional e contempla qualificação profissional, formação inicial e continuada, capacitação, aperfeiçoamento, especialização e atualização.</div>
          <div class="legal-box"><strong>Importante:</strong> a legislação federal não cria uma categoria única denominada “curso livre”. Este certificado registra a conclusão da formação ofertada pelo ENAT e não equivale a diploma de graduação, pós-graduação ou curso técnico, nem substitui habilitação, licença ou certificação profissional exigida por órgão competente.</div>
          <div class="legal-box disclaimer"><strong>Para o contexto do trânsito:</strong> quando aplicável, a formação deve ser interpretada em conjunto com a legislação específica da profissão de instrutor de trânsito e demais normas vigentes. O certificado comprova a participação e o aproveitamento na formação descrita neste documento.</div>
        </div>
      </div>
      ...
      <div class="registration">ENAT — Ensino Neuroeducacional Aplicado ao Trânsito<br>Certificado nº ${safe(certificateId)} • Emitido em ${safe(date)}</div>
    </div>
  </div>
</div>
...
function AccessBlockedPage({ onSubscribe, status }) {
  const statusLabel = status === "pending"
    ? "Sua assinatura está aguardando a conclusão do pagamento."
    : status === "paused"
      ? "Sua assinatura está pausada."
      : status === "canceled"
        ? "Sua assinatura foi cancelada."
        : "O acesso profissional ainda não está ativo.";

  return (
    <div style={{ maxWidth: "920px", margin: "0 auto" }}>
      <div className="panel" style={{ padding: 30, textAlign: "center" }}>
        <div style={{ fontSize: 48, marginBottom: 8 }}>🔒</div>
        <small style={{ fontWeight: 900, letterSpacing: ".08em", color: "#55BFEF" }}>ENAT — ACESSO PROFISSIONAL</small>
        <h1 style={{ margin: "8px 0" }}>Assinatura necessária</h1>
        <p style={{ color: "#5f6b7a", lineHeight: 1.65, maxWidth: 680, margin: "0 auto 18px" }}>
          {statusLabel} Para utilizar Alunos, Agenda, Aulas, HSI-DOTH-P, RPA ÚNICO e Financeiro, ative o Plano Profissional ENAT.
        </p>
        <div style={{ display: "inline-flex", gap: 8, alignItems: "center", padding: "10px 16px", borderRadius: 999, background: "#f2f8fd", border: "1px solid #dbeaf5", fontWeight: 800 }}>
          Plano Profissional • R$ 50,00/mês
        </div>
        <div style={{ marginTop: 22 }}>
          <button type="button" onClick={onSubscribe}>💳 VER ASSINATURA</button>
        </div>
      </div>
    </div>
  );
...
function SubscriptionPage({ user, subscription, onRefresh, onBack }) {
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState("");

  const active = subscription?.status === "active";
  const pending = subscription?.status === "pending";
  const statusText = active ? "ATIVA" : pending ? "PENDENTE" : (subscription?.status || "NÃO CONTRATADA").toUpperCase();
  ...
  async function subscribe() {
    ...
    try {
      ...
      if (data?.already_active) {
        setMsg("Sua assinatura já está ativa.");
        onRefresh?.();
        return;
      }
      if (data?.init_point) {
      ...
      throw new Error(data?.error || "O Mercado Pago não retornou o link de assinatura.");
    } catch (err) {
      console.error(err);
      setMsg(err?.message || "Não foi possível iniciar a assinatura. Verifique a configuração do Mercado Pago no Supabase.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div style={{ maxWidth: "980px", margin: "0 auto" }}>
      <div className="panel" style={{ padding: 28 }}>
        <button type="button" className="link" onClick={onBack}>VOLTAR</button>
        <small style={{ fontWeight: 900, letterSpacing: ".08em", color: "#55BFEF" }}>ENAT — ASSISTENTE DO INSTRUTOR</small>
        <h1 style={{ margin: "6px 0 8px" }}>Assinatura ENAT</h1>
        <p style={{ lineHeight: 1.65, color: "#5f6b7a", maxWidth: 780 }}>
          Plano recorrente para manter o acesso profissional ao Assistente do Instrutor, aos recursos do sistema e aos cursos incluídos.
        </p>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 16, marginTop: 20 }}>
          <div className="panel" style={{ padding: 22, border: "2px solid #55BFEF" }}>
            <small style={{ color: "#55BFEF", fontWeight: 900, letterSpacing: ".08em" }}>PLANO PROFISSIONAL</small>
            <div style={{ fontSize: 38, fontWeight: 900, color: "#1f3b68", marginTop: 8 }}>R$ 50,00<span style={{ fontSize: 15, fontWeight: 700 }}>/mês</span></div>
            <p style={{ color: "#5f6b7a", lineHeight: 1.55 }}>Cobrança recorrente pelo Mercado Pago.</p>
            <ul style={{ lineHeight: 1.8, paddingLeft: 20 }}>
              <li>Dashboard e gestão do instrutor</li>
              <li>Alunos, Agenda e Aulas</li>
              <li>HSI-DOTH-P e RPA ÚNICO</li>
              <li>Financeiro</li>
              <li>Cursos, avaliações e certificados</li>
              <li>Novos conteúdos disponibilizados pelo ENAT</li>
            </ul>
            <button type="button" onClick={subscribe} disabled={busy || active} style={{ width: "100%" }}>
              {busy ? "ABRINDO MERCADO PAGO..." : active ? "✓ ASSINATURA ATIVA" : pending ? "CONTINUAR ASSINATURA" : "💳 ASSINAR POR R$ 50,00/MÊS"}
            </button>
          </div>

          <div className="panel" style={{ padding: 22, background: "#f8fbfe" }}>
            <h2 style={{ marginTop: 0 }}>Seu acesso</h2>
            <p><strong>Status:</strong> <span style={{ color: active ? "#18764a" : "#a06400", fontWeight: 900 }}>{statusText}</span></p>
            <p><strong>Valor:</strong> R$ 50,00/mês</p>
            <p><strong>Provedor:</strong> Mercado Pago</p>
            {subscription?.next_due_date && <p><strong>Próxima cobrança:</strong> {new Date(subscription.next_due_date).toLocaleDateString("pt-BR")}</p>}
            {subscription?.last_payment_status && <p><strong>Último pagamento:</strong> {subscription.last_payment_status}</p>}
            <button type="button" className="secondary" onClick={onRefresh}>↻ ATUALIZAR STATUS</button>
            <p style={{ fontSize: 13, color: "#6d7885", lineHeight: 1.5, marginTop: 16 }}>
              O acesso não é liberado pelo simples retorno do navegador. A ativação depende da confirmação do Mercado Pago e da atualização do status no servidor.
            </p>
          </div>
        </div>

        <div className="panel" style={{ marginTop: 16, padding: 20 }}>
          <h2 style={{ marginTop: 0 }}>Cursos e conteúdos adicionais</h2>
          <p style={{ color: "#5f6b7a", lineHeight: 1.6, marginBottom: 0 }}>
            O plano inclui os cursos definidos pelo ENAT como conteúdo de assinatura. Cursos, formações e palestras especiais poderão ser disponibilizados como compras avulsas, sem alterar o plano mensal.
          </p>
        </div>

        <div className="panel" style={{ marginTop: 16, padding: 20 }}>
          <h2 style={{ marginTop: 0 }}>Contato ENAT</h2>
          <p style={{ color: "#5f6b7a" }}>Dúvidas sobre assinatura, cursos, certificados ou suporte:</p>
          <a href="mailto:enat.neuroeducacao@gmail.com" style={{ fontWeight: 800, color: "#2457a6" }}>enat.neuroeducacao@gmail.com</a>
        </div>

        {msg && <p className="msg" style={{ marginTop: 14 }}>{msg}</p>}
      </div>
    </div>
  );
}
