...
function HsiDothPAssessment({ user, selectedStudent }) {
  ...
  const selectedStudentData = students.find(item => item.id === selectedId);
  ...
  return (
    <div>
      ...
      {selectedId && (
        <div className="panel">
          <h2>Análise individual{selectedStudent ? ` — ${selectedStudentData.full_name}` : ""}</h2>
      ...
      <div className="panel" style={{ background: "#f7faff", border: "1px solid #cfe0f3" }}>
        ...
        {records.length === 0 ? (
          ...
          <>
            ...
            {selectedStudentData && latest && (
              <>
                <h3>Orientação para {selectedStudentData.full_name}</h3>
                <p>
