    ...
    <div>
      <div className="panel" style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: 0 }}>
              {readOnly ? "Consulta somente leitura dos registros da aula." : "Siga as fases na ordem e finalize com a avaliação pedagógica."}
            </p>
          </div>
      ...
      <div className="panel">
        <div style={{
          ...
          <div>
            <h2 style={{ marginBottom: "4px" }}>Avaliação pedagógica</h2>
        ...
        {evaluationComplete && (
          <div style={{
            ...
            <div style={{ border: "1px solid #dfe5ec", borderRadius: "10px", padding: "13px", background: "#fbfcfe" }}>
              <div style={{ fontWeight: 800 }}>Próxima aula</div>
      ...
      <section className="panel" style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <h2 style={{ margin: 0 }}>Avaliação pedagógica</h2>
        ...
        {resolvedPedagogicalEvaluation ? (
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {pedagogicalBand?.description || "Resultado pedagógico da aula."}
                  </div>
                </div>
          ...
          <div style={{
            ...
            <div style={{ marginTop: "5px", fontSize: "12px", opacity: 0.75 }}>
              Nenhuma avaliação pedagógica estruturada foi encontrada nesta aula.
            </div>
          </div>
        )}
      </section>
      ...
      <section className="panel" style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: "5px 0 0", opacity: 0.7 }}>
              Comparação automática com a avaliação pedagógica anterior do mesmo aluno.
            </p>
          </div>
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ marginTop: "4px", fontSize: "12px", opacity: 0.72 }}>
                É necessário ter pelo menos uma aula anterior do mesmo aluno com avaliação pedagógica completa.
              </div>
            </div>
          </div>
        ) : (
...
function HsiDothPAssessment({ user, selectedStudent }) {
  ...
  const dimensions = [
    {
      key: "decision",
      label: "Decisão",
      description: "Analisa alternativas e escolhe respostas seguras diante das situações de trânsito.",
      recommendation: "Reforçar análise de alternativas, antecipação e escolha de respostas seguras."
    },
    {
      key: "organization",
      label: "Organização",
      description: "Organiza ações, procedimentos e prioridades antes e durante a condução.",
      recommendation: "Trabalhar planejamento da ação, sequência de procedimentos e organização das tarefas."
    },
    {
      key: "time",
      label: "Tempo",
      description: "Administra tempo, velocidade de resposta e ritmo da condução sem precipitação.",
      recommendation: "Trabalhar ritmo adequado, antecipação e controle do tempo de resposta."
    },
    {
      key: "humanization",
      label: "Humanização",
      description: "Considera o outro, a convivência, a responsabilidade e o impacto das próprias escolhas.",
    ...
    },
    {
      ...
      description: "Integra autorregulação, comportamento, emoções e resposta diante de situações de pressão.",
  ...
  useEffect(() => {
    ...
    async function loadLessons() {
      ...
      if (error) {
        setMsg(error.message || "Não foi possível carregar as aulas.");
        return;
      }
  ...
  const diagnosis = complete
    ? `Resultado ${classification(average).toLowerCase()}, com média ${average.toFixed(2)}/5 (${normalized}/100). ` +
      (development.length
        ? `Fatores prioritários: ${development.map(item => `${item.label} (${item.score}/5)`).join(", ")}.`
        : "Nenhum fator apresentou nota igual ou inferior a 3; recomenda-se consolidar o desempenho.")
    : "Conclua os cinco fatores para gerar o diagnóstico.";

  const recommendation = complete
    ? development.length
      ? development.map(item => `${item.label}: ${item.recommendation}`).join(" ")
      : "Manter as competências desenvolvidas e ampliar gradualmente a complexidade das situações observadas."
    : "Conclua a avaliação antes de gerar recomendações.";
  ...
  async function saveAssessment() {
    ...
    if (!lessonId) {
      setMsg("Selecione uma aula concluída para vincular a avaliação.");
      return;
    }
    if (!complete) {
    ...
    try {
      ...
      if (error) throw error;
      if (!data?.id) throw new Error("A avaliação não foi confirmada pelo banco.");
      ...
      setMsg(error?.message || "Não foi possível salvar a avaliação.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div>
      <div className="panel">
        <h1>HSI-DOTH-P</h1>
        <p>
          Avaliação inicial integrada ao Assistente do Instrutor.
          Nesta etapa, o resultado é vinculado a uma aula existente e gravado
          nos dados da própria aula, sem criação de tabela nova.
        </p>
        ...
        {loading ? (
          ...
          <>
            ...
            <label style={{ marginTop: "12px" }}>Aula concluída</label>
            <select value={lessonId} onChange={e => setLessonId(e.target.value)}>
              <option value="">Selecione uma aula</option>
              {lessons.map((lesson, index) => (
                <option key={lesson.id} value={lesson.id}>
                  Aula {lessons.length - index} — {lesson.started_at ? new Date(lesson.started_at).toLocaleString("pt-BR") : "sem data"}
                </option>
              ))}
            </select>
          </>
      ...
      <div className="panel">
        ...
        <p>
          Atribua uma nota de <strong>1 a 5</strong> para cada dimensão observada.
          A escala desta primeira versão é operacional e será refinada quando
      ...
      <div className="panel" style={{
        ...
        <div className="grid">
          <div className="metric">
            <div style={{ fontSize: "12px", opacity: 0.7 }}>MÉDIA</div>
        ...
        <div style={{
          ...
          <h3 style={{ marginTop: 0 }}>Diagnóstico inicial</h3>
          <p>{diagnosis}</p>
