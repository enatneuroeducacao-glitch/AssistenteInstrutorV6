...
function LessonDetails({ lesson, onBack }) {
  ...
  const pedagogicalBand = pedagogicalEvaluation?.classification || null;
  const pedagogicalDiagnosis = pedagogicalEvaluation?.diagnosis || null;
  const nextLessonRecommendation = pedagogicalEvaluation?.next_lesson_recommendation || null;
  const nextLessonObjective = pedagogicalEvaluation?.next_lesson_objective || null;
              ...
              {Object.entries(evaluationLabels).map(([key, label]) => (
                <div key={key} style={{
                  ...
                  <div style={{
                    ...
                    {pedagogicalEvaluation[key] ?? "—"}<span style={{ fontSize: "12px", opacity: 0.65 }}> / 5</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{
              ...
              <strong>
                {pedagogicalEvaluation.average != null
                  ? `${Number(pedagogicalEvaluation.average).toFixed(2)} / 5`
                  : "Não informada"}
              </strong>
            </div>
